...
> - v4: 정식 review-loop R2(redteam) 반영 — **R1의 B-1·M-3 보완이 틀렸음을 정정**하고 새 blocker 2건을 해소.
>   - **B2-1**(blocker): 결정 7의 R1 B-1 bullet이 serving·console-api를 한데 묶어 "둘 다 `python:3.12-slim`, healthcheck는 `urlopen('.../health')`"라 쓴 것이 **console-api에 대해 이중으로 틀림** — console-api base는 slim이 아니라 `python:3.12-alpine`(`Dockerfile.api:14` 직접 확인)이고 `/health` 라우트가 **없다**(`api.py`는 `/console/versions`·`/console/versions/{v}`·`/console/audit`·`/console/approve`·`/console/rollback` 5개뿐, 직접 확인). B-1 bullet을 **서비스별로 분리 정정**: serving=slim+`urlopen('.../health')`, console-api=alpine(busybox wget 또는 python 둘 다 가능)+엔드포인트 `/console/versions?fs=vitals`(빈 상태 200 — `console-api.yaml:87` "빈 클러스터에서도 200 확인됨"). base 이미지 `[확인됨]` 태그를 serving=slim/console-api=alpine으로 분리. 내부모순(체인은 console-api를 healthcheck 불요로, bullet은 healthcheck 부여) 해소 — console-api는 실제로 동작하는 healthcheck를 갖고, front-nginx→console-api는 `service_healthy`로 확정.
>   - **B2-2**(blocker): 결정 2 M-3의 "auditdb=named volume이면 소유권 자동 정합" 처방이 **Docker 동작과 어긋남** — Docker named volume은 런타임 uid로 chown하지 않고 마운트 지점이 이미지에 없으면 root:root로 생성. `Dockerfile.api:25`는 `/app/deploy/artifacts`만 mkdir하고 `/app/auditdb`는 **미생성** → root 소유 → uid 10001이 sqlite 생성 불가. 게다가 `AuditStore`가 **모듈 임포트 시점** 인스턴스화(`service.py:30`)돼 생성자가 `create_all` 즉시 실행(`audit.py:54-56`) → console-api가 부팅 전 임포트에서 크래시(감사 append-only 불변식 붕괴). "named volume 자동 회피" 문구를 **삭제**하고 `/app/auditdb`를 콘솔 이미지 Dockerfile에서 `mkdir + chown 10001`로 선생성하는 구체 처방으로 교체(artifacts에 요구한 것과 동일 수준).
>   - **M2-1**(major): 결정 7의 front-nginx "alpine nginx `[확인됨]`"이 mislabel(인용한 `console-web/Dockerfile:14`는 console-web 확인일 뿐 신규 front-nginx가 아님) → `[우리 결정]`으로 격하하고 front-nginx base를 alpine으로 명시 결정. front-nginx healthcheck는 비-게이트라 붕괴 파급 없음도 명시.
>   - **M2-2**(major): 결정 7 B-R0-3의 seed precondition "즉시 종료"는 `app.py`에 startup/lifespan 훅이 **없고** 로드가 첫 `/health`에서 lazy 트리거(`app.py:31` `FastAPI(...)` lifespan 미지정, `:72-73` state() lazy — 직접 확인)라 자동 성립 안 함 → app.py에 boot precondition 훅(lifespan) **신규 추가**가 필요함을 결정 7에 의존으로 명시(이게 "serving 재사용" 범위를 넘는 코드 변경임을 정직히 기록).
>   - **minor**: 결정 5 auditdb 볼륨 선택을 M-3/B2-2 chown 처방 확정에 연결(더 이상 "bind vs named 열어둠" 아님), 알려진 한계의 `Dockerfile:3` "no pandas" 인용 정정(`:15`가 pandas==2.3.3 설치 — 실질 주장 no xgboost만 유지), front-nginx→web 과도기 502 창을 한계에 명시, grafana←prometheus·renderer←grafana depends 순서·provisioning 볼륨 승계를 서비스 맵에 확인.
> - v5: 정식 review-loop R3(redteam) 반영 — R2 보완(B2-1·B2-2·M2-1·M2-2)이 실물 대조로 **정확히 해소**됐음이 확인됐고(가짜 수렴 아님), v4에도 남아 있던 새 층을 정정.
>   - **B3-1**(blocker): 결정 7·알려진 한계가 front-nginx를 console-web `service_healthy`로 게이트한다고 명시하나, 결정 7 healthcheck bullet은 serving·console-api·front-nginx **세 서비스만** test를 규정하고 **console-web healthcheck는 어디에도 규정 안 함**. console-web 이미지엔 `HEALTHCHECK`가 없어(`console-web/Dockerfile:1-22` 직접 확인 — `nginxinc/nginx-unprivileged:alpine`, uid 101, listen 8080, SPA 폴백 try_files) → `service_healthy` condition **영원히 미충족** → front-nginx 미기동 → 호스트 공개 진입점(front-nginx:80) 통째로 죽어 브라우저 콘솔 UI 불가용(B-1·B2-1과 동종 함정, 대상만 console-web으로 이동). **정정(a 채택)**: 결정 7에 console-web healthcheck를 신규 명세 — alpine busybox `wget`으로 `test: wget -q --spider http://localhost:8080/`, `/`는 SPA 폴백 `try_files`라 항상 200(`console-web/Dockerfile:19` 확인). front-nginx가 게이트하는 **모든** 대상(console-api·console-web)에 실재·작동하는 healthcheck가 갖춰져 "healthcheck 없이 service_healthy로 게이트"하던 모순 제거. (b 완화 대안은 502 창을 남기므로 기각 — healthy 게이트가 502 창 자체를 없앰.)
>   - **M3-1**(major): 결정 7 기동 체인 번호 서술 "(1) serving healthy 후 (2) console-api"가 M-2 완화(console-api→serving은 `service_started`)와 **내부모순** — 핸드오프가 번호 체인만 보고 `service_healthy`로 걸면 M-2가 제거한 결함(캘리브레이션 300s 콘솔 불가용·seed 누락 시 진단 불가) 부활. **정정**: 번호 순서 서술을 폐기하고 기동 체인을 **방향별 condition으로 재서술**(serving=무의존, console-api→serving=`service_started`, front-nginx→console-api·console-web=`service_healthy`) — 번호가 아니라 condition이 권위임을 명시.
>   - **minor**: 서비스 맵 front-nginx "빌드 소스"에 base=alpine 명시(M2-1 반영), auditdb named volume 재-chown 한계(B2-2 chown 처방은 빈 최초 volume에만 적용 → 이전 root 소유 volume 있으면 재배포 시 `docker volume rm` 선행 필요)를 알려진 한계에 한 줄 추가.
...
- **★필수(핸드오프 확정) — 마운트 쓰기 권한 정합 (M-3 / B2-2 정정)**: 이건 "옵션"이 아니라 **임계 쓰기 경로를 막을 수 있는 필수 항목**이다. K8s는 `fsGroup: 10001`로 마운트 볼륨 소유 그룹을 비-root uid에 맞춰 쓰기를 보장했으나 [확인됨: `console-api.yaml:33,37` `fsGroup: 10001`, `:53` `runAsUser: 10001`], **Compose에는 fsGroup에 해당하는 재귀 chown 장치가 없다**. 그런데 임계 쓰기가 두 곳에서 일어난다 — (a) 승인 시 심링크 스왑(alias 교체) [확인됨: `serve/bundle.py:34-39` `set_alias`가 tmp 심링크 생성→`os.replace`; console 승인 경로 `retrain/deploy.py`가 이를 호출], (b) 감사 DB 스키마 생성/append [확인됨: `audit.py:54-56` 생성자가 `Base.metadata.create_all`, `:59-62` `append` INSERT].
  - **artifacts (bind mount)**: 비-root(uid 10001)로 실행하는데 호스트 폴더 소유권이 어긋나면 심링크 스왑이 permission denied. **호스트 폴더 소유권 + 컨테이너 `user:` 지정을 정합**시킨다(예: 호스트 `chown 10001 ./deploy/artifacts` 또는 seed·컨테이너 uid를 일치). export가 만든 파일 소유권과 컨테이너 실행 uid 정합도 여기서 확정 [검증 필요: WSL2 bind mount uid 매핑].
  - **auditdb (B2-2 정정 — R1의 "named volume이면 자동 회피"는 틀렸다)**: R1은 "auditdb는 named volume으로 두면 Docker가 소유권을 컨테이너 uid에 맞춰 회피된다"고 확정했으나 **이는 Docker 동작과 어긋난다** — Docker named volume은 런타임 uid로 chown하지 않고, 마운트 지점이 이미지에 있으면 그 소유권을 복사·없으면 **root:root로 생성**한다. `Dockerfile.api`는 `/app/deploy/artifacts`만 mkdir하고 `/app/auditdb`는 **만들지 않는다** [확인됨: `Dockerfile.api:25` `RUN mkdir -p /app/deploy/artifacts`뿐]. 그 결과 auditdb 볼륨이 root 소유로 생성돼 uid 10001이 sqlite 파일을 만들지 못한다. 게다가 이 실패는 요청 처리 전에 터진다 — `AuditStore`가 **모듈 임포트 시점** 인스턴스화되고(`service.py:30` `audit: AuditStore = AuditStore(...)` 모듈 전역), 그 생성자가 `create_all`을 **즉시** 실행하므로(`audit.py:54-56`), `api.py:13`이 `from sepsis.console import service`를 임포트하는 순간 root 소유 디렉토리에 sqlite를 만들려다 `OperationalError`로 **부팅 전 임포트에서 크래시**한다(감사 테이블 생성조차 실패 = append-only 불변식 붕괴). 따라서 **콘솔 이미지 Dockerfile에서 `/app/auditdb`를 `RUN mkdir -p /app/auditdb && chown 10001:10001 /app/auditdb`로 선생성**해 named volume이 이 이미지 소유권(uid 10001)을 복사하도록 만든다(artifacts에 요구한 것과 동일 수준의 구체 처방) [우리 결정]. 대안(엔트리포인트 init chown = 비-root 실행과 충돌·root 실행 = 결정 6 위배)보다 이미지 선생성이 가장 단순하고 결정 6(비-root)과도 정합. auditdb를 bind mount로 둘 경우엔 artifacts와 동일하게 호스트 `chown 10001` 필요.
...
- **★소유권 처방은 결정 2(M-3/B2-2)에서 확정**: auditdb는 named volume이든 bind든 **소유권 정합이 자동으로 되지 않는다** — named volume도 마운트 지점(`/app/auditdb`)이 이미지에 없으면 root:root로 생성돼 uid 10001이 sqlite를 못 만든다(게다가 `AuditStore`가 모듈 임포트 시점 `create_all`을 실행해 부팅 전 크래시). 따라서 결정 2의 확정 처방(**콘솔 이미지 Dockerfile에서 `/app/auditdb`를 `mkdir + chown 10001` 선생성**)을 따른다. 이전 "bind vs named 열어둠"은 종결(volume 종류와 무관하게 chown 처방이 본질).
- **검토 요청 항목**: 영속·백업이 되는지(volume 종류 무관), auditdb 소유권 처방(결정 2 B2-2)이 적용돼 create_all이 uid 10001로 성공하는지.
...
- **결정**: `depends_on` + healthcheck로 기동 순서를 보장하되, **번호 순서가 아니라 각 의존의 `condition`을 방향별로 명시**한다(번호 서술로 뭉뚱그리면 M-2·M3-1 완화가 사라지므로 **condition이 권위**). 기동 체인 전체를 한 곳에서 모순 없이 재서술한다:
  - **serving** — 다른 서비스에 의존하지 않는다(체인 맨 앞). 부팅 시 torch 로드 + 300-trial 캘리브레이션을 마치면 `/health` healthy.
  - **console-api → serving** — `condition: service_started`로만 의존한다(**`service_healthy` 아님** — M-2/M3-1). console-api 부팅 lifespan은 serving을 호출하지 않고 alias↔감사만 화해하므로, serving healthy를 기다리면 (a) 캘리브레이션 ~300s 동안 콘솔 불가용, (b) seed 누락으로 serving이 stuck-unhealthy면 console-api가 영영 안 떠 진단 불가 — 아래 ★가용성 결합 방지 M-2 참조. serving이 뜨지 못해도 console-api는 떠서 "active version 없음"을 표시해야 한다.
  - **front-nginx → console-api** — `condition: service_healthy`. console-api healthcheck `/console/versions?fs=vitals`가 빈 상태 200이라 값싸게 준비 확인 → 미준비 upstream 프록시 502 창 제거(B2-1).
  - **front-nginx → console-web** — `condition: service_healthy`. console-web healthcheck는 아래 ★에서 신규 명세한다(B3-1). 정적 SPA라 준비 확인이 값쌈.
  - healthcheck 엔드포인트는 서비스별로 다르다 — serving=`/health`, console-api=`/console/versions?fs=vitals`, console-web=`/`, front-nginx=`/`(base·도구·엔드포인트 서비스별 차이는 아래 ★ B-1·B2-1·B3-1). 부팅 예산은 K8s startupProbe(~300s)에 대응하는 `start_period`로.
  - **방향 주의**: console-api→serving은 `service_started`(느슨), front-nginx→console-api·console-web은 `service_healthy`(엄격)로 **방향이 다르다**. 과거 "(1) serving healthy 후 (2) console-api" 번호 서술은 M-2 완화와 모순이라 폐기했다(M3-1).
- **★healthcheck는 이미지에 존재하는 도구로만 구현 — 서비스별 분리 (B-1 / B2-1 / B3-1 정정)**: Compose healthcheck는 K8s `httpGet`(kubelet이 컨테이너 밖에서 수행)과 달리 **컨테이너 안에서 명령을 실행**한다. 따라서 이미지에 실제로 있는 도구 + 실제로 있는 엔드포인트로만 짜야 한다. **R1의 B-1 bullet은 serving과 console-api를 한데 묶어 "둘 다 `python:3.12-slim` + `urlopen('.../health')`"라 썼는데, console-api에 대해 base 이미지와 엔드포인트 둘 다 틀렸다(B2-1).** 서비스별로 분리 확정한다:
  - **serving** — base `python:3.12-slim` [확인됨: `deploy/Dockerfile:4`], curl·wget 미설치(pip로 torch/fastapi/uvicorn/numpy/pandas/scipy/pydantic만) [확인됨: `deploy/Dockerfile:12-15`], `/health` 라우트 실재 [확인됨: `app.py:110-115`]. → healthcheck는 slim에 있는 python 인터프리터로 `test: ["CMD","python","-c","import urllib.request,sys; sys.exit(0 if urllib.request.urlopen('http://localhost:8000/health').status==200 else 1)"]`. (`/health` 실패 시 500→`urlopen`이 HTTPError raise→비-0→올바로 unhealthy — false-healthy 경로 없음.)
  - **console-api** — base는 slim이 아니라 **`python:3.12-alpine`** [확인됨: `Dockerfile.api:14` `FROM python:3.12-alpine`]. alpine엔 busybox `wget`이 있고 pip로 python도 있어 둘 다 가능. 그러나 **`/health` 라우트가 없다** — 라우트는 `/console/versions`·`/console/versions/{v}`·`/console/audit`·`/console/approve`·`/console/rollback` 5개뿐 [확인됨: `console/api.py:44,49,54,77,87`]. 따라서 엔드포인트는 `/health`가 아니라 **`/console/versions?fs=vitals`**(빈 상태에서도 200 반환 — K8s 프로브도 동일 경로 사용) [확인됨: `console-api.yaml:81,87,95` 프로브 `path: /console/versions?fs=vitals`, `:87` 주석 "빈 클러스터(번들 0개)에서도 200 확인됨"]. → `test: ["CMD","wget","-qO-","http://localhost:8000/console/versions?fs=vitals"]`(busybox wget) 또는 python urllib 동형. R1처럼 `urlopen('.../health')`를 쓰면 404→HTTPError→영구 unhealthy로 **B-1과 같은 부류 결함이 console-api에 재발**하므로 금지.
  - **console-api는 실제로 동작하는 healthcheck를 갖는다** — 따라서 내부모순(R0 체인은 console-api를 healthcheck 불요로, R1 bullet은 부여) 해소: console-api healthcheck는 유효하며, **front-nginx→console-api는 `service_healthy`로 확정**한다(console-api healthcheck가 빈 상태 200으로 값싸고 안정적이라 부작용 없이 nginx가 미준비 api로 프록시하는 502 창을 없앤다). 반면 console-api→serving 방향은 M-2대로 `service_started`(부팅 의존 완화) — 이 둘은 방향이 다르다.
  - **front-nginx (M2-1 정정)** — front-nginx는 이 스택에서 신규로 만드는 이미지이고 base가 아직 미결정이다. R1이 인용한 `console-web/Dockerfile:14 nginxinc/nginx-unprivileged:alpine`은 **console-web 확인일 뿐 front-nginx가 아니므로** `[확인됨]`은 mislabel → `[우리 결정]`으로 격하하고 **front-nginx base = alpine nginx로 명시 결정**한다(busybox `wget` 가용 → `test: ["CMD","wget","-qO-","http://localhost:80/"]`) [우리 결정]. front-nginx healthcheck는 기동 게이트의 뒤끝(맨 앞이 아님)이라 붕괴 파급이 없다(비-게이트) — serving/console-api 게이트만 백본. (curl을 이미지에 추가 설치하는 대안은 슬림/alpine 유지 원칙과 어긋나 기각.)
  - **console-web (B3-1 신규) — front-nginx가 `service_healthy`로 게이트하는데 healthcheck가 없어 게이트 미충족 함정**: front-nginx는 console-web을 `service_healthy`로 기다리는데(위 기동 체인·아래 알려진 한계), console-web 이미지에는 `HEALTHCHECK`가 **없다** [확인됨: `console-web/Dockerfile:1-22` 전체에 HEALTHCHECK 지시어 부재]. Compose에서 `depends_on: console-web: {condition: service_healthy}`인데 대상에 healthcheck가 없으면 그 condition은 **영원히 미충족** → front-nginx 미기동 → 서비스 맵상 호스트 공개는 front-nginx:80뿐이고 console-web/api는 내부이므로 **브라우저 콘솔 UI(핵심 산출물) 진입점 통째로 불가용**. B-1(serving)·B2-1(console-api)이 잡아온 "게이트가 의존하는 healthcheck가 실재/작동 안 해 기동 백본 붕괴" 함정의 4번째 서비스 재발이다. → console-web base는 **`nginxinc/nginx-unprivileged:alpine`**(비-root uid 101, **8080** listen) [확인됨: `console-web/Dockerfile:14,12,19,20,21`]이라 busybox `wget`이 있으므로 healthcheck를 **명세**한다: `test: ["CMD","wget","-q","--spider","http://localhost:8080/"]`(또는 `wget -qO- http://localhost:8080/ || exit 1`). 엔드포인트 `/`는 SPA 폴백 `try_files $uri $uri/ /index.html`이라 번들·라우트와 무관하게 **항상 200** [확인됨: `console-web/Dockerfile:19` baked default.conf]. 포트는 80이 아니라 **8080**(비-root라 80 바인드 불가) [확인됨: `console-web/Dockerfile:12,17` "listen 8080 — 비-root 라 80 바인드 불가"]. 이로써 front-nginx가 게이트하는 **모든** 대상(console-api·console-web)에 실재·작동하는 healthcheck가 갖춰져 "healthcheck 없이 `service_healthy`로 게이트"하던 현 모순이 제거된다. (대안: front-nginx→console-web을 `service_started`로 완화 — front-nginx는 자칭 비-게이트 뒤끝이고 502 창은 이미 한계에 기재됐으나, healthy 게이트가 502 창 자체를 없애므로 **명세(a)를 채택**하고 완화는 기각.)
- **근거 + 출처등급**:
...
- **★seed 미완 시 실패 비대칭 (B-R0-3)**: seed는 결정 2의 수동 선작업이나, 누락 시 두 서비스 거동이 갈린다 — **console-api는 graceful**(빈 alias·빈 audit이면 `_reconcile_or_seed`가 어느 분기도 안 타고 no-op 리턴) [확인됨: `service.py:135-150` — `alias_target=None & last=None`이면 무동작]. **그러나 serving은 graceful 아님** — 빈 alias 해석/번들 로드에서 예외 → `/health`가 반복 500(**stuck-unhealthy**: uvicorn 프로세스는 살아 있고 `/health`만 계속 실패) [검증 필요: `app.py` `_load_all`의 실제 예외 메시지·경로]. (용어 주의: 이건 진짜 crash-loop이 아니다 — 프로세스가 반복 종료·재기동하는 crash-loop은 아래 precondition explicit-exit + `restart`가 결합될 때에야 성립한다.) 따라서 serving에 **명시적 seed precondition 체크**(부팅 초입에 alias 존재 확인 → 없으면 "seed 필요: `uv run python -m scripts.h4.h4s_export_bundle vitals`" 명확 에러로 즉시 종료)를 두어, stuck-unhealthy(원인 불명확) 대신 **읽을 수 있는 실패**로 만든다 [우리 결정].
  - **★boot 훅이 없어 자동 성립 안 함 — app.py lifespan 신규 추가 의존 (M2-2)**: 이 "부팅 초입 즉시 종료"는 지금 코드에선 **자동으로 성립하지 않는다**. `app.py`에는 startup/lifespan 훅이 없고(`FastAPI(title="sepsis-serving", version="h4s")` — lifespan 미지정) [확인됨: `app.py:31`], 번들 로드는 **첫 요청(`/health`)에서 lazy 트리거**된다(`state()`가 `"pred" not in _S`일 때 `_load_all`) [확인됨: `app.py:72-73`, `:110-115` health()가 state() 호출]. 즉 seed가 없어도 프로세스는 뜨고, 실패는 첫 healthcheck에서 500으로 나타나 여전히 stuck-unhealthy다. "부팅 초입 즉시 종료"를 실현하려면 **app.py에 lifespan(startup) 훅을 신규 추가**해 alias 유무를 부팅 시 확인하고 없으면 즉시 비-0 종료해야 한다. 이는 "serving 이미지 재사용" 범위를 넘는 **serving 코드 변경**이므로 정직하게 결정 7의 의존으로 기록한다 — 이 훅을 넣지 않으면 seed 누락은 계속 stuck-unhealthy로 남는다 [우리 결정]. (체크 위치·메시지·종료코드 구현은 핸드오프. seed 명령은 인자 없이 실행하면 기본 순회에서 `vitals_labs`까지 돌다 RuntimeError로 비-0 종료할 수 있으므로 **`vitals` 단일 인자**를 권장 — 결정 2·8 참조.)
...
- **검토 요청 항목**: serving `start_period`가 최악 부팅(캘리브레이션 스로틀)을 커버하는지, console-api가 serving 없이도 부팅되는지(승인 시에만 serving 호출하므로 console-api→serving 의존은 `service_started`로 느슨해야 — M-2), seed precondition 체크가 stuck-unhealthy를 명확 실패로 바꾸는지(그러려면 app.py에 lifespan boot 훅 신규 추가 필요 — M2-2), healthcheck `test`가 **서비스별로 올바른 도구+엔드포인트**로 짜였는지(serving=slim/python urllib/`/health`, console-api=alpine/wget 또는 urllib/`/console/versions?fs=vitals`, front-nginx=alpine/wget/`/` — B-1·B2-1), console-api alpine 이미지에서 `/console/versions?fs=vitals`가 빈 상태 200을 반환하는지 실측, `/app/auditdb` chown 처방 적용 후 uid 10001로 create_all 성공하는지 실측(B2-2).
...
| front-nginx | nginx conf (신규, base=alpine — M2-1) | 80 (호스트 공개) | 없음 | conf |
...
>
> **모니터링 서브스택 기동·볼륨 (기존 `deploy/monitoring/docker-compose.yml` 승계 — minor)**: 통합 스택은 기존 모니터링 compose의 depends·볼륨 구조를 그대로 승계한다 — `grafana`는 `prometheus`·`renderer` 둘 다에 `depends_on`(렌더 콜백·데이터소스 순서), `renderer`는 무의존 [확인됨: `deploy/monitoring/docker-compose.yml:25` `depends_on: [prometheus, renderer]`, `:40-42` renderer]. grafana provisioning·dashboards 볼륨(`../grafana/provisioning`·`../grafana/dashboards`, RO)도 승계 [확인됨: `:34-36`]. prometheus 타깃만 `serving:8000`으로 재배선(결정 4), 나머지 서비스 관계·프로비저닝 볼륨은 불변.
...
- **front-nginx 과도기 502 창** — front-nginx→web/api를 `service_healthy`로 걸어 기동 초기 502는 대체로 없앴으나(결정 7 B2-1), 런타임에 upstream(console-api/web)이 재시작·일시 unhealthy가 되는 동안엔 front-nginx가 준비 안 된 upstream으로 프록시해 짧은 502 창이 생길 수 있다. POC 한계로 명시(재시도/대기 프록시 튜닝은 백로그) [우리 결정].
...
- **auditdb named volume 재-chown 안 됨 (재배포 시 운영노트)** — 결정 2/B2-2의 `mkdir + chown 10001` 처방은 named volume이 **최초(빈) 마운트일 때에만** 이미지 마운트지점 소유권(uid 10001)을 복사한다. 이전 실행에서 root 소유로 만들어진 volume이 이미 있으면 이미지를 재빌드해도 그 volume은 root 소유로 남아 uid 10001이 sqlite create_all에 실패한다(부팅 전 크래시). POC 최초 배포는 fresh volume이라 무해하나, **재배포·이미지 교체 시 소유권이 어긋난 기존 volume은 `docker volume rm <auditdb_volume>` 선행 후 재기동**해야 한다 [우리 결정]. (근본 해결은 엔트리포인트 chown이나 fsGroup 등가물이지만 결정 6 비-root 원칙·POC 범위상 백로그.)
- **수치 재현 = torch/numpy 버전 고정 전제** — serving 스택은 **GRU-only**(이미지에 xgboost·mlflow 미설치) [확인됨: `deploy/Dockerfile:15` pip 설치 목록에 xgboost·mlflow 없음]. (주의: `Dockerfile:3` 주석은 "no MLflow/pandas/xgboost"라 하지만 `:15`가 실제로는 `pandas==2.3.3`을 설치한다 — drift import 체인용. 따라서 정확한 실질 주장은 "no xgboost/mlflow, GRU-only"이며 pandas는 설치됨. minor: 주석 인용을 pandas 부분에서 신뢰하지 말 것.) 재현성은 torch·numpy 등 서빙 의존 버전을 uv.lock과 일치시키는 것에 달림 [확인됨: `deploy/Dockerfile:9-15` torch==2.12.1·numpy==2.5.0 고정 + "uv.lock 과 정확히 일치" 주석]. (v2 이전판의 "best_iter/xgboost 3.3.0" 서술은 이 GRU 스택엔 해당 없음 — xgboost는 별도 벤치 하니스 소관이라 삭제, M-4.)
