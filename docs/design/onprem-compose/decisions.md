...
> **상태**: 설계부 v4 — R2 반영(R1이 틀리게 보완한 B-1·M-3를 정정; blocker 2·major 2·minor 4 해소).
> **개정 이력**
...
- **결정**: `depends_on` + healthcheck로 순서 보장 — (1) serving이 healthy(부팅 캘리브레이션 완료) 후, (2) console-api(lifespan에서 alias↔감사 화해), (3) front nginx는 web+api가 **healthy** 뒤(`service_healthy` — 아래 B2-1 참조: console-api healthcheck가 `/console/versions?fs=vitals`로 빈 상태 200이라 값싸게 준비 확인 가능 → 502 창 제거). serving healthcheck 엔드포인트는 `/health`, console-api는 `/console/versions?fs=vitals`(base·엔드포인트가 서비스별로 다름 — B2-1). 부팅 예산은 K8s startupProbe(~300s)에 대응하는 `start_period`로. **단 console-api→serving 방향의 의존은 `service_healthy`가 아니라 `service_started`로 완화**한다(아래 ★가용성 결합 방지 M-2 참조 — front-nginx→console-api의 `service_healthy`와 방향이 다름).
- **★healthcheck는 이미지에 존재하는 도구로만 구현 — 서비스별 분리 (B-1 / B2-1 정정)**: Compose healthcheck는 K8s `httpGet`(kubelet이 컨테이너 밖에서 수행)과 달리 **컨테이너 안에서 명령을 실행**한다. 따라서 이미지에 실제로 있는 도구 + 실제로 있는 엔드포인트로만 짜야 한다. **R1의 B-1 bullet은 serving과 console-api를 한데 묶어 "둘 다 `python:3.12-slim` + `urlopen('.../health')`"라 썼는데, console-api에 대해 base 이미지와 엔드포인트 둘 다 틀렸다(B2-1).** 서비스별로 분리 확정한다:
