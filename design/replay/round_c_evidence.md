...
- **Grafana UI 자체는 안 띄움.** 이 환경에 prometheus/grafana 스택(compose)이 없어, 패널이 **읽는
  메트릭 시계열**(`/metrics` 의 `serve_pred_prob_latest`)을 직접 관측·캡처했다. 패널은 이 시계열에
  대한 쿼리일 뿐이라 데이터 계층은 입증됐으나, **렌더링된 그래프 스크린샷은 없음**(다음에 스택 띄우면 보강).
...
- `reports/replay_p000018_gauge_curve.tsv` — p000018 Gauge 시간 궤적 40 표본(위험도 선 raw).
- 이 문서.
...
(2.3), 기본 OFF 가 카디널리티 footgun 을 막는다(2.4). 남은 빚 = Grafana 렌더 스크린샷(스택 부재).
