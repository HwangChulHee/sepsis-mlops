...
> 주의(프로메테우스 노출 방식): 이 두 지표의 `# HELP`·`# TYPE` **헤더 라인**은 지표가 등록돼 있는 한 게이트 OFF여도 `/metrics`에 **항상** 나타난다. OFF에서 사라지는 것은 **라벨이 붙은 샘플 라인**뿐이다. 따라서 성공기준은 "지표 이름 substring 부재"가 아니라 "**라벨 샘플 라인의 부재/불변**"으로 판정해야 한다. 순진한 `assert "serve_input_feature_value" not in text`는 헤더 때문에 OFF에서도 실패한다.

- **성공기준 A1-a (OFF → 피처별 샘플 라인 부재)**: 피처(`feature=...` 라벨)가 붙은 **샘플 라인**이 한 줄도 나타나지 않는다 — 구체적으로 `serve_input_feature_value_bucket{feature=...}` / `serve_input_feature_value_count{feature=...}` / `serve_input_feature_value_sum{feature=...}` 와 `serve_input_missing_total{feature=...}` 라인이 `/predict`를 20회 호출한 뒤에도 0줄이다(HELP/TYPE 헤더 라인만 남는다). 즉 라벨 조합 샘플이 관측되지 않는다.
- **성공기준 A1-b (대조, ON → 샘플 라인 출현·증가)**: 같은 서버를 **ON으로 기동**하고 같은 요청을 보내면 위 `feature=...` 라벨 샘플 라인들이 `/metrics`에 **나타나고**, `serve_input_feature_value_count{feature=...}`가 호출 횟수만큼 **증가**한다(결측 피처가 있으면 `serve_input_missing_total{feature=...}`도 증가).
- 두 상태의 이 **라벨 샘플 라인 유무·증감** 차이가 "게이트가 실제로 부가 계측을 끈다"를 관측 증명한다.
...
- **성공기준 A2-b (상태 진행 불변)**: 같은 환자의 여러 타임스텝을 순서대로 보내 **환자별 상태가 시퀀스에 걸쳐 누적되게** 한 뒤, 동일한 시퀀스를 ON·OFF 서버에 흘리면, 매 응답 p 시퀀스가 두 상태에서 동일하다. 게이트가 예측에 쓰는 상태 진행을 건드리지 않는다.
