...
- **이미지 2개**: `sepsis-console:a`(uvicorn console.api:app, **alpine ~175MB** — torch/numpy/pandas
  제거: console 은 모델을 안 돌리고 deploy.py/bundle.py 의 무게 import 를 lazy 화함), `sepsis-console-web:a`(nginx-unprivileged+dist).
- **합성 번들 생성기**(`scripts/h4/gen_synth_bundle.py`)는 console 파드가 아니라 **serve 파드**에서 실행한다
  (console 이미지엔 torch 가 없다 — serve 이미지에 있고 같은 PVC 를 마운트). `kubectl exec <serve-pod> -- python /tmp/gen.py`.
