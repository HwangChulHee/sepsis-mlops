...
- **대상**: `design/h4_serving_decisions.md` v2 (개정 이력 v2 — HOLD 1 + 등급 정정)
- **검토일**: 2026-06-28
- **판정**: ✅ **PASS — HOLD 0건.** v1 HOLD 해소, pdm 등급 정정, 신규 모순 없음. → **다음은 `h4_serving_handoff.md`** (비차단 권고 7건 흡수). 마이크로 nit 1건은 핸드오프 흡수.

### 회귀 검증 (요청 3항목)

**1. HOLD (번들 원자성) → ✅ 해소.**
- 결정 1(`h4_serving_decisions:32`): "설정은 **단일 run을 선택**하고 그 run에서 **model+featureset+μ/σ·fill·clip+τ+input_dim을 원자적으로(통째) 로드** — model·featureset을 독립 노브로 두지 않는다." 교체는 `gru_vitals ↔ gru_vitals_labs`처럼 **run 단위**.
- **불일치 번들 구조적으로 불가**: 설정이 단일 run을 가리키고 그 run 안에 매칭된 model+stats+τ+input_dim이 묶여 있으므로(H2가 run 단위 번들로 저장), `gru+vitals_labs 모델에 vitals 통계`를 섞을 진입로가 없다. skew 통로 차단.
- PASS #2(`:120`)에 **"model+featureset+전처리+τ+input_dim이 동일 run 출처(불일치 번들 불가) assert"** 추가. 검토요청(`:38`)도 정합. ✓

**2. pdm 등급 → ✅ 하향, 잔재 없음.**
- 결정 7(`:96`)·결정 8(`:105`): `[확인됨]`→**`[우리 경험: pdm-mlops — CC 1차 확인 불가, 핸드오프 인라인]`**. 결정 5(`:76`)도 `[우리 결정 · pdm-mlops 경험]`(애초 [확인됨] 아님).
- grep 결과 **`[확인됨]`에 묶인 pdm 주장 0건**(개정 이력의 정정 설명 1줄만 매치). 과대표기 잔재 없음. ✓
- 남은 `[확인됨]`(GRU 최고·vitals 전이·m2m·τ·A 동결·마스크 OFF)은 전부 repo/H3/코드로 검증 가능 — 정당.

**3. 신규 모순 → 없음.**
- run 단위 ↔ 결정 4: run 번들이 μ/σ·fill·clip을 포함 → 결정 4의 "A 동결 전처리"가 같은 run에서 로드되어 정합.
- run 단위 ↔ 결정 1 교체가능성: featureset 미결 반영한 교체는 여전히 가능(run 스왑), 단 원자적. "재설정 가능 운영환경" 메시지 유지.
- 자립성: 인프라 [우리 경험]+핸드오프 인라인 약속(결정 8 `:107`)으로 일관.

### 마이크로 nit (핸드오프 흡수 — 비차단)
- **입력 스키마 ↔ run 번들**(결정 5 `:79`): run 교체 시 기대 입력 필드(vitals 9 vs vitals_labs 18)도 **로드된 번들의 featureset에서 파생**해야 함(스키마도 번들에 종속). 핸드오프에 1줄 명시(원자성의 자연 귀결).

### 1차 확인 (v1에서 검증, 변동 없음)
hidden-state causal 동치(단방향 GRU) · ffill 스트리밍 동치(상태 NaN 초기화 전제) · 정규화 글로벌 A-train이라 미래 누수 없음 · H2 아티팩트 로드 가능 — 전부 [확인됨: 코드 대조].

**결론: HOLD 0 → `h4_serving_handoff.md` 작성으로 진행.** 비차단 권고 7건 + nit 1건은 핸드오프에서 흡수(DDD 재라운드 불필요).
