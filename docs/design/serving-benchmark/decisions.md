...
> **상태**: 설계부 v4 — 라운드 3 blocker(B-R3-1)·major(M-R3-1) 보완, 재검토 대기.
> **개정 이력**
> - v1: 초안 (설계부).
...
- **★XGB 아티팩트·전처리 소스 식별 (MJ1·B-R3-1)**: XGB 최소 앱은 승격 별칭이 아니라 **MLflow run 디렉토리**에서 로드한다 — `deploy/artifacts/`엔 GRU 별칭만 있고 XGB 승격 별칭은 없다 [확인됨: `deploy/artifacts/`에 `gru_vitals*`만, xgb 없음]. 챔피언을 재현하려면 **세 조각이 모두** 필요하다 — `.ubj` + `tau` + **`best_iter`**:
  ...
  3. **★`best_iter` (early-stopping 절단) — 챔피언 재구성의 load-bearing 의존**: H2/H3 챔피언은 부스터 전체 트리가 아니라 **early-stopping best_iteration까지만** 쓴다. 서빙이 `.ubj`를 로드해 `Booster.predict()`를 그냥 부르면 **전체 트리**로 추론 → 챔피언과 **다른 모델**(latency 부풀림·확률 이동·tau 오정렬). 챔피언 확률·latency를 재현하려면 `iteration_range=(0, best_iter+1)`로 절단해야 한다 [확인됨: `src/sepsis/train/tree.py:69-75` `booster_predict`가 xgboost일 때 `iteration_range=(0, int(best_iter)+1)`로 절단]. 채점 경로도 이 절단을 명시 주입한다 [확인됨: `eval/crosssite.py:60,65` `score_tree_frozen(booster, model_name, best_iter, tau, …)`가 `booster_predict(..., best_iter)` 호출; `scripts/h3/h3b_crosssite.py:144,158,162`가 best_iter를 주입].
     - **소스 = MLflow run metric `metrics.best_iter`** — `.ubj`에도 `preprocess.json`에도 **없다** [확인됨: preprocess.json keys에 best_iter 없음; `scripts/h2/h2b_train_trees.py:183` `mlflow.log_metrics({…, "best_iter": …})`로 metric에만 로깅, `:189` `log_dict`(preprocess.json)엔 tau·featureset만; `h3b_crosssite.py:144` `r.get("metrics.best_iter")`로 run metric에서 읽음]. 따라서 XGB 서빙 소스 식별은 **`.ubj`(모델) + preprocess.json(`tau`) + run metric(`best_iter`)** 세 곳이다.
     - **설계부 수준**: 여기까진 "best_iter가 챔피언 입력 의존이며 소스가 run metric이다"까지만 확정한다. best_iter를 어떻게 읽어 XGB 서버 predict에 주입(하드코딩 vs 벤치 설정 vs run metric 조회)할지 **값 주입 경로 구현은 핸드오프로 위임**한다.
  - (run id 하드코딩 대신 벤치 설정으로 주입할지는 위 3의 주입 경로와 함께 핸드오프.)
- **근거 + 출처등급**:
...
1. **예측/추론 로직 불변**: 프로덕션 서빙의 **예측/추론 로직**(`predict`·응답 dict·`_row_from`)·콘솔 미변경(grep 강제). **단 결정 1 격리 예외 — 관측성 전용 env 게이트(per-feature 루프·drift window add 가드, 예측/추론·`LATENCY` 관측 불변)는 허용**(NB3). XGB 벤치는 독립 최소 서빙으로, 기존 GRU **예측 경로** 오염 없음. XGB 최소 앱 응답은 `{patient_id, p, alarm, featureset}` **전체 스키마** 복제(M1). **XGB 서빙 소스 = `.ubj` + `tau` + `best_iter`** — `mlruns/1/…/model/xgboost_{vitals,vitals_labs}.ubj`(모델) + 각 run `preprocess.json`(`tau`) + run metric `metrics.best_iter`(early-stopping 절단), XGB 서빙은 `iteration_range=(0, best_iter+1)`로 절단해야 챔피언 확률·latency 재현(MJ1·B-R3-1; best_iter는 `.ubj`·preprocess.json 아닌 run metric에만 존재, 값 주입 경로는 핸드오프).
...
- 예상 blocker 후보(레드팀이 팔 자리): ①XGB lookback 버퍼가 계약을 안 깨고 서버측 상태로 성립하는지·초기(8행 미만) 요약의 skew ②arm-2 잔차가 실제로 network+직렬화로 좁혀지는지 ③관측성 게이트가 예측 로직 불변을 grep으로 증명 가능한지 ④통제 arm 잔차의 state 기여를 메모리 분해가 실제로 분리하는지 ⑤순차 실행이 하드웨어 상태(캐시·thermal) 차이를 남기는지 ⑥best_iter 값 주입 경로(핸드오프)가 챔피언 절단을 실제로 재현하는지.
