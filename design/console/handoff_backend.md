...
> **상태**: 명세부 v2 — 레드팀 라운드 1 반영(B1 버전 식별자 규약=디렉토리명 단일화, B2 전파경로 이중접두사 제거, MJ1 git_commit→retrain.json, MJ2 do_orm_execute bulk 차단, mn1~3). 검토: `handoff_backend_review.md`.
...
- `_read_meta(version_dir)` → `meta.json` 파싱(필드: `featureset·hp·input_dim·tau·version·trained_on·run_id`). **`git_commit` 없음**(deploy.py:49-52).
- `_read_retrain(version_dir)` → `retrain.json` 파싱(필드: `epochs·val_loss·b_split_seed·n_*·run_id·git_commit`). **`git_commit`의 단일 출처**(deploy.py:62-64).
...
**규약 — 정규 식별자 = 버전 디렉토리명**: 감사 `from_version`/`to_version`, 화해(`_reconcile_or_seed`) 비교, 롤백 타겟, 전파 타겟, 그리고 쓰기 API의 버전 입력은 **전부 버전 디렉토리명** `gru_<fs>@<v>`(예: `gru_vitals@v3`) **하나로 통일**한다. 맨버전(`v3`)은 핸드오프 어디에도 **단독으로 등장하지 않는다.**

**근거(코드 정합)**: `deploy.active_version(fs)`는 `os.readlink`로 **디렉토리명**을 반환하고(deploy.py:71-73), `deploy.rollback`/`deploy.set_active`→`bundle.set_alias`는 그 문자열을 **상대 심링크 타겟으로 그대로** 쓴다(bundle.py:35, deploy.py:77·95). 즉 alias 계약의 양끝이 이미 디렉토리명이므로 그 표현으로 통일하면 화해·롤백·전파에 **변환이 0회**다. (맨버전 정규화 헬퍼를 택하면 읽기마다 strip·쓰기마다 prefix 재부착이 생겨 B2류 이중접두사 버그가 재발 — 디렉토리명 통일을 채택.)
...
- **금지**: `ARTIFACTS / f"gru_{fs}@{...}"` 형태의 접두사 재조립은 본 핸드오프 전 구간에서 금지하고 `_version_dir(version_id)`만 쓴다. (옛 v1의 line 102·179 모순이 여기서 제거된다.)
- 읽기 API `GET /console/versions/{version}`의 `{version}` 경로 파라미터도 디렉토리명(`gru_vitals@v3`)을 받는다 — `_version_dir`로 직접 경로화.
...
class AuditEvent(Base):
    ...
    from_version  = Column(String, nullable=True)               # prev(이전 활성) — **버전 디렉토리명**(gru_<fs>@<v>), 콜드스타트 BOOTSTRAP은 NULL
    to_version    = Column(String, nullable=False)              # 대상(새 활성) — **버전 디렉토리명**(active_version 반환 규약과 동일)
    run_id        = Column(String, nullable=True)               # 메모상자: MLflow 링크 키(박제). 출처=meta.json.run_id(deploy.py:52). 없으면 NULL
    git_commit    = Column(String, nullable=True)               # 메모상자: 출처=**retrain.json**(deploy.py:64). meta.json엔 없음(MJ1)
...
| `GET /console/versions/{version}?fs=vitals` | `{version}` = 디렉토리명(`gru_vitals@v3`). `_version_dir`로 경로화해 그 dir의 `validation.json`·`retrain.json`·`meta.json` 읽어 게이트 수치 + 재학습 상세 + MLflow deep-link | 읽기 전용. `meta.json.run_id` 있으면 링크, 없으면 폴백 표시(6-A) |
...
- **버전 식별자 = 디렉토리명(B1)**: `champion`·`challenger`·`archived`·`version` 입력이 모두 디렉토리명(`gru_<fs>@<v>`)으로 표현돼 감사 `to_version`·롤백 타겟과 같은 표현을 쓴다(상단 "버전 식별자 규약").
...
```python
# config.py — CONSOLE_FEATURESETS 출처 고정(mn2): 모듈 상수, 환경변수 override 허용
CONSOLE_FEATURESETS = os.environ.get("CONSOLE_FEATURESETS", "vitals").split(",")  # 기본 ["vitals"]

@asynccontextmanager
async def lifespan(app):
    for fs in CONSOLE_FEATURESETS:                    # 출처 = console/config.py(mn2)
...
def _reconcile_or_seed(fs: str):
    ...
    if last is None and alias_target is not None:
        ...
        # 심링크 소실 + 감사 이력 존재(mn3): alias=현재 활성 권위인데 champion 부재.
        # 없는 champion을 감사로 날조하지 않는다(거짓 복원 금지) — 운영자 경보 + UI "활성 없음(alias 소실)".
...
def _propagate_and_confirm(fs: str, *, timeout_s=10, interval_s=0.5) -> str:
    ...
    target = deploy.active_version(fs)                # ★ 타겟 = 현재 alias(그 swap의 버전 아님, MJ-r5) — 디렉토리명
...
1. **감사 append-only (두 경로)**: (a) `audit.append`로 INSERT는 되나 로드한 레코드 인스턴스 UPDATE/DELETE 후 flush는 `PermissionError`(before_flush). (b) **bulk 우회 차단(MJ2)**: `session.execute(update(AuditEvent).values(...))`·`session.execute(delete(AuditEvent))`(및 `query.update()/delete()`)도 `PermissionError`(do_orm_execute). `ts`는 UTC.
2. **감사 스키마**: APPROVE 레코드에 `gate_snapshot`이 version dir `validation.json`의 사본으로 박히고, `gate_passed`가 `no_regression`과 일치. `from_version`·`to_version`이 **디렉토리명**(`gru_<fs>@<v>`)이고 `git_commit`이 **retrain.json**에서 채워짐(meta.json 아님, NULL 아님 — MJ1). `actor_unverified` 기본값 존재, `verified_subject`는 NULL.
...
6. **직렬화 + 식별자 규약(면 2·B1)**: 같은 featureset 동시 approve 2건이 직렬화돼 둘째가 갱신된 active를 prev로 읽음(prev 갈라짐 없음). 감사에 `gru_<fs>@V1→gru_<fs>@V2`·`gru_<fs>@V2→gru_<fs>@V3`로 남고(전부 디렉토리명) `V1→V3` 오염 없음. archived 링크가 `to`=다음 레코드 `from`으로 끊김 없이 이어짐.
7. **화해(면 1·B1)**: alias가 감사 `last_active`와 다른 상태로 부팅 시 `RECONCILE` 레코드 1건이 `from=감사최종·to=실제alias`로 기록되고, archived 도출이 보존됨. **정상 승인 1회 후 재기동 시에는 RECONCILE이 생기지 않음**(`last.to_version`·`alias_target` 둘 다 디렉토리명이라 거짓양성 없음 — B1). 콜드스타트(감사 빔+champion 존재)는 `BOOTSTRAP` seed 1건(`actor_unverified="system"` — mn1). alias=None인데 이력 존재면 거짓 복원 없이 경보만(mn3). **화해/seed가 라우팅 개시 전 완료**(lifespan 순서).
