...
## 3.5 동시성 버스트 — 재해석 (요청률 여유 ≠ 동시성 여유)

헤드라인 "700× 여유"는 **요청률(RPS)** 기준이다. 별개 질문: **"수백 환자가 거의 동시에 스코어링되는 순간(예: 정각 배치 라운딩)"** 의 동시성 버스트를 감당하나?

**결론: 이미 상한이 잡혀 있다 — 새 측정 불요.**
- 전이 버스트(N명이 t=0에 동시 도착 → 배수 → idle)는 **지속 풀스피드(N명 상시 in-flight)보다 harsh하지 않다.** 지속이 끊임없이 큐를 채우는 반면 버스트는 한 번 배수 후 쉰다.
- 따라서 §2·§3의 지속 풀스피드 런이 버스트 최악값을 **위에서 bound**한다: N=200 지속에서 p99 350ms·에러 0이면, 200-동시 버스트는 그보다 쉽다.
- 단일 200-버스트 배수 추정: 1워커 ≈1.4ms/req(700rps 역수) × 200 ≈ **마지막 환자 ~286ms** — §3 관측 p50(300ms)과 정합. 패혈증은 시간 단위 진행이라 임상상 무시 수준.

**정직한 한계 (도메인 [검증 필요])**: 이 버스트가 **실제로 일어나는지**는 이 데이터로 판정 불가 — PhysioNet 2019는 익명화로 벽시계 timestamp가 없고 `ICULOS`(상대 경과시간)만 있다 [확인됨: `config.py:55` EXCLUDED ICULOS·HospAdmTime]. 환자 간 측정이 벽시계로 몰리는지(정각 배치) 분산되는지(스트리밍)는 EMR 스케줄에 달렸고 병원마다 다르다. 즉 **최악값은 감당 확인됐으나, 그 최악이 실재하는 빈도는 데이터 밖 도메인 사실이다.**

> 면접 한 문장: *"동시성 버스트를 별도 측정하려다, 지속 풀스피드가 전이 버스트보다 harsh해 기존 데이터가 이미 최악을 bound함을 깨달았다 — 게다가 익명화 데이터라 버스트 실재는 원리상 검증 불가라, 최악 감당은 확인하고 실재성은 정직한 한계로 남겼다."*
...
- **동시성 버스트 실재성**: 최악(N-동시)의 감당은 지속 풀스피드로 확인됐으나(§3.5), 실제 EMR이 정각 배치로 요청을 몰리게 하는지는 익명화 데이터로 판정 불가 — 도메인 [검증 필요].
