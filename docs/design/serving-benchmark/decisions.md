# Serving-Benchmark 설계결정문서 (DDD) — 아키텍처별 운영비용 벤치마크 (featureset 결합 배포 프로파일)
...
> **상태**: 설계부 v5 — 라운드 4 blocker(B-R4-1: best_iter 소스를 `.ubj` 임베드로 정정·실로드 검증) 보완, 재검토 대기.
> **개정 이력**
> - v1: 초안 (설계부).
...
> - v4: 라운드 3 — B-R3-1(XGB 챔피언 재구성의 `best_iter` 의존 식별 — 소스=run metric `metrics.best_iter`, `iteration_range=(0, best_iter+1)` 절단; 결정 2·PASS 게이트 1), M-R3-1(XGB `serve_predict_latency_seconds` 경계 = 버퍼→lookback_summary 재구성 + booster.predict를 함께 감쌈 = GRU predict의 StreamPreprocessor 포함과 대칭; 결정 3·PASS 게이트 2), mn-R3-1(제목에 "(featureset 결합 배포 프로파일)") 반영.
> - v5: 라운드 4 — **B-R4-1**(best_iter 소스 정정: v4의 "소스=run metric `metrics.best_iter`, `.ubj`에도 없다 [확인됨]"이 틀림 — 이 저장소 mlruns는 artifacts-only pruned로 run metric 미영속(`grep best_iter mlruns/`=0), 반대로 `.ubj`는 best_iteration을 임베드. **실제 booster 로드로 복구 확인**(`b.best_iteration`=105/149, xgboost 3.3.0)해 소스를 **`.ubj` 임베드**로 확정; 결정 2 3항·PASS 게이트 1), mn-R4-1(주입 best_iter 유효 양수 계약 — falsy/음수 전체-트리 폴백 금지) 반영.
...
- **★XGB 아티팩트·전처리 소스 식별 (MJ1·B-R3-1·B-R4-1)**: XGB 최소 앱은 승격 별칭이 아니라 **MLflow run 디렉토리**에서 로드한다 — `deploy/artifacts/`엔 GRU 별칭만 있고 XGB 승격 별칭은 없다 [확인됨: `deploy/artifacts/`에 `gru_vitals*`만, xgb 없음]. 챔피언을 재현하려면 **세 정보 조각이 모두** 필요하되(`.ubj` 모델 + `tau` + **`best_iter`**), best_iter가 `.ubj`에 임베드돼 있어 **실질 소스 파일은 두 개**(`.ubj` + `preprocess.json`)다:
  1. **부스터 `.ubj`** = `mlruns/1/3e21f380b380422d8d52f78904e54ad4/artifacts/model/xgboost_vitals.ubj`(9) · `mlruns/1/fe64aac54f344999baa217f56e4e963c/artifacts/model/xgboost_vitals_labs.ubj`(18) [확인됨: 파일 실재].
  2. **alarm 임계 `tau`** = 각 run의 `artifacts/preprocess.json`(keys: `featureset, scale_pos_weight, tau, hp, note`) [확인됨: JSON keys `['featureset','scale_pos_weight','tau','hp','note']` 실측]. XGB는 트리 NaN-native라 정규화(mean/std) 불필요 — preprocess.json에 정규화 통계 없음이 정상 [확인됨: keys에 mean/std 없음].
  3. **★`best_iter` (early-stopping 절단) — 챔피언 재구성의 load-bearing 의존**: H2/H3 챔피언은 부스터 전체 트리가 아니라 **early-stopping best_iteration까지만** 쓴다. 서빙이 `.ubj`를 로드해 `Booster.predict()`를 그냥 부르면 **전체 트리**로 추론 → 챔피언과 **다른 모델**(latency 부풀림·확률 이동·tau 오정렬). 챔피언 확률·latency를 재현하려면 `iteration_range=(0, best_iter+1)`로 절단해야 한다 [확인됨: `src/sepsis/train/tree.py:69-75` `booster_predict`가 xgboost일 때 `iteration_range=(0, int(best_iter)+1)`로 절단]. 채점 경로도 이 절단을 명시 주입한다 [확인됨: `eval/crosssite.py:60,65` `score_tree_frozen(booster, model_name, best_iter, tau, …)`가 `booster_predict(..., best_iter)` 호출; `scripts/h3/h3b_crosssite.py:144,158,162`가 best_iter를 주입].
     - **소스 = `.ubj`에 임베드된 `best_iteration` (1차 소스 — 로드한 booster에서 직접 복구, run 스토어 불필요)**: XGBoost 네이티브 저장(`h2b_train_trees.py:99` `model.get_booster().save_model`)은 early-stopping best_iteration을 learner attribute로 `.ubj`에 보존한다. **실제 로드로 복구 확인** [확인됨: `xgb.Booster().load_model(<.ubj>)` 후 `b.best_iteration` = **105**(`.../3e21f380…/model/xgboost_vitals.ubj`) / **149**(`.../fe64aac5…/model/xgboost_vitals_labs.ubj`); `b.attr("best_iteration")`·`b.attributes()['best_iteration']`도 동일값; 총 트리 `b.num_boosted_rounds()` = 136 / 180 이라 절단이 실질적(전체≠best_iter) — xgboost 3.3.0으로 실측]. 이 임베드 값이 **챔피언이 쓴 best_iter와 동일**함도 코드로 확인 — 로깅된 metric `best_iter`와 `.ubj` 임베드 값이 **같은 `model.best_iteration`에서 나온다** [확인됨: `train/tree.py:49-51` `best_iteration()`이 `getattr(model, "best_iteration")` 반환 = metric 소스(`h2b_train_trees.py:93,183` `c["best_iter"]`); `save_model`이 같은 모델의 `get_booster()`를 저장 = `.ubj` 임베드]. 따라서 XGB 서빙은 run 스토어 없이 `.ubj`만 로드해 `b.best_iteration`으로 절단값을 **자립적으로** 얻는다.
     - **run metric `metrics.best_iter`는 이 저장소에서 미영속 — 옛 "유일 소스" 단정 폐기**: H3 실행 당시엔 스토어에 `metrics.best_iter`가 있어 `h3b_crosssite.py:144` `r.get("metrics.best_iter")`가 읽었으나, **이 저장소의 mlruns는 artifacts-only로 pruned되어 run 디렉토리에 `metrics/`·`meta.yaml`이 없다** [확인됨: `mlruns/1/3e21f380…/` = `artifacts/preprocess.json` + `artifacts/model/*.ubj` 2개뿐, `metrics/`·`meta.yaml` 부재, `grep -r best_iter mlruns/` = **0건**]. 핸드오프가 설계대로 "run metric 조회"를 구현하면 이 저장소에선 아무 값도 못 얻는다. 그래서 소스를 run metric에서 **`.ubj` 임베드로 정정**한다(자립·재현 가능). preprocess.json엔 best_iter 없음은 그대로 [확인됨: keys `['featureset','scale_pos_weight','tau','hp','note']`].
     - **설계부 수준**: 여기까진 "best_iter가 챔피언 입력 의존이며 소스가 `.ubj` 임베드 `best_iteration`이다"까지만 확정한다. 로드한 booster에서 `b.best_iteration`을 읽어 XGB 서버 predict에 주입하는 **구현(읽기 시점·주입 방식·유효성 체크)은 핸드오프로 위임**한다.
     - **★주입 best_iter 유효성 계약 (mn-R4-1)**: 절단은 best_iter가 유효 양수일 때만 걸린다 — `tree.py:74` `rng = (0, int(best_iter)+1) if best_iter and best_iter >= 0 else None`이라 falsy(0/None)·음수면 `None`으로 폴백해 **무성(無聲) 전체-트리** 추론(챔피언 아님). `.ubj` 임베드 값이 105/149 양수임은 확인됐으나, 서빙 계약에 "복구한 best_iter는 유효 양수(≥1)여야 하며, 복구 실패 시 조용한 전체-트리 폴백을 금지하고 명시적 실패로 처리"를 못박는다 [우리 결정].
  - (`.ubj` 파일 경로는 `mlruns/1/<run_id>/artifacts/model/…`로 run id에 종속 — run id를 하드코딩할지 벤치 설정으로 줄지는 핸드오프. best_iter는 그 `.ubj` 안에 임베드돼 있어 별도 스토어 조회 없이 로드 시 함께 복구된다.)
- **근거 + 출처등급**:
...
- **XGB 최소 앱에도 동일 계측점 필수**: XGB 서빙도 `/predict` 내부에 **동일한 `serve_predict_latency_seconds` 히스토그램(같은 이름)**을 둔다 [우리 결정]. 그래야 두 서빙에서 (client 벽시계, 서버 히스토그램, 잔차 분해)가 **같은 관측 경계**로 비교된다.
- **★XGB 히스토그램 관측 경계 = 버퍼→lookback_summary 재구성 + booster.predict를 함께 감쌈 (M-R3-1 — GRU와 대칭)**: GRU `predict()`는 내부에 전처리(`StreamPreprocessor.step`: ffill→fill_mean→clip→z-score)를 포함하므로 [확인됨: `serve/predictor.py:44` `z = self.pre.step(pid, row)`가 `predict()` 안에서 실행], GRU 히스토그램은 "전처리 + 신경망 forward"를 함께 잰다. XGB가 이와 대칭이 되려면 `serve_predict_latency_seconds`가 **환자별 8행 버퍼 → `data/features.lookback_summary` 재구성(윈도우 7종 통계, 결정 2) + `booster.predict`(best_iter 절단)를 함께** 감싸야 한다 [우리 결정]. XGB 앱이 요약 재구성을 핸들러에 인라인하고 `booster.predict`만 히스토그램에 넣으면 두 "서버 히스토그램"이 서로 다른 범위(GRU=전처리 포함 / XGB=추론만)를 재게 되어 **NB1형 비대칭이 다른 형태로 재발**한다. 따라서 XGB 히스토그램 경계 = "입력 재구성 + 추론"(GRU의 "전처리 포함 predict"와 동형)으로 못박는다.
- **근거 + 출처등급**:
...
1. **예측/추론 로직 불변**: 프로덕션 서빙의 **예측/추론 로직**(`predict`·응답 dict·`_row_from`)·콘솔 미변경(grep 강제). **단 결정 1 격리 예외 — 관측성 전용 env 게이트(per-feature 루프·drift window add 가드, 예측/추론·`LATENCY` 관측 불변)는 허용**(NB3). XGB 벤치는 독립 최소 서빙으로, 기존 GRU **예측 경로** 오염 없음. XGB 최소 앱 응답은 `{patient_id, p, alarm, featureset}` **전체 스키마** 복제(M1). **XGB 서빙 소스 = `.ubj`(모델 + 임베드 `best_iteration`) + `preprocess.json`(`tau`)** — `mlruns/1/…/model/xgboost_{vitals,vitals_labs}.ubj` + 각 run `preprocess.json`. `best_iter`는 **`.ubj`에 임베드**되어 로드한 booster에서 직접 복구(`b.best_iteration` = 105/149 실측, run 스토어 불필요). XGB 서빙은 `iteration_range=(0, best_iter+1)`로 절단해야 챔피언 확률·latency 재현(MJ1·B-R3-1·B-R4-1). **복구한 best_iter는 유효 양수여야** 하며 falsy/음수 시 조용한 전체-트리 폴백 금지·명시적 실패(mn-R4-1). (run metric `metrics.best_iter`는 H3 당시엔 있었으나 이 저장소 mlruns는 artifacts-only pruned으로 미영속 — 소스를 `.ubj` 임베드로 정정. 값 복구·주입 구현은 핸드오프.)
2. **계측**: latency는 **client 벽시계(`HttpSender` 래핑) + 서버 히스토그램(`serve_predict_latency_seconds`, `predict()` 구간만)** 병행, p50/p95/p99. **client − 히스토그램 = client−server 잔차(= network + 직렬화 + 핸들러 후처리)** — 이 잔차를 arm-1에서 "network"로 헤드라인 금지(NB1·mn1). **network 추정치는 arm-2(부가계측 OFF)에서만 헤드라인**(잔차가 network+직렬화로 좁혀짐), 핸들러 후처리분은 (arm-1 잔차 − arm-2 잔차) 차분으로 별도 귀인. throughput은 `replay_many` 동시 부하. 메모리는 서빙 프로세스 RSS·peak. **XGB 최소 앱도 GRU와 동일 관측 경계** — GRU 히스토그램이 전처리(`StreamPreprocessor.step`) 포함 predict를 재므로, XGB `serve_predict_latency_seconds`는 **버퍼→`lookback_summary` 재구성 + `booster.predict`(best_iter 절단)를 함께** 감싼다(M-R3-1; booster.predict만 감싸면 GRU와 관측 범위 비대칭 재발).
...
- v4(라운드 3 보완): redteam blocker **B-R3-1**(결정 2·PASS 게이트 1 — XGB 챔피언 재구성의 `best_iter` 의존 식별: 소스 = MLflow run metric `metrics.best_iter`(`.ubj`·preprocess.json 아님), 서빙은 `iteration_range=(0, best_iter+1)`로 절단해야 챔피언 확률·latency 재현; 값 주입 경로는 핸드오프 위임) 해소 [확인됨: `train/tree.py:69-75`·`eval/crosssite.py:60,65`·`h3b_crosssite.py:144,158,162`·`h2b_train_trees.py:183,189` 실측]. major **M-R3-1**(결정 3·PASS 게이트 2 — XGB `serve_predict_latency_seconds` 경계 = 버퍼→`lookback_summary` 재구성 + `booster.predict`를 함께 감쌈, GRU predict의 `StreamPreprocessor.step` 포함과 대칭 [확인됨: `serve/predictor.py:44`]) 해소. minor **mn-R3-1**(제목에 "(featureset 결합 배포 프로파일)") 반영.
- v5(라운드 4 보완): redteam blocker **B-R4-1**(결정 2 3항·PASS 게이트 1 — best_iter 소스를 run metric에서 **`.ubj` 임베드 `best_iteration`으로 정정**) 해소. v4의 미검증 "`.ubj`에도 없다 [확인됨]"·"소스=run metric" 단정 폐기. **핵심: 실제 booster 로드로 검증** [확인됨: `xgb.Booster().load_model` 후 `b.best_iteration` = 105(`xgboost_vitals.ubj`)/149(`xgboost_vitals_labs.ubj`), `b.attributes()`·`b.attr("best_iteration")` 동일, `b.num_boosted_rounds()`=136/180 → 절단 실질적, xgboost 3.3.0]. run metric 미영속도 실측 [확인됨: `mlruns/1/…` = `.ubj`+`preprocess.json`뿐, `metrics/`·`meta.yaml` 부재, `grep -r best_iter mlruns/`=0]. metric·임베드 동일 출처 코드 확인 [확인됨: 둘 다 `model.best_iteration`, `train/tree.py:49-51`·`h2b_train_trees.py:93,183,99`]. minor **mn-R4-1**(주입 best_iter 유효 양수 계약, `tree.py:74` falsy/음수 폴백 방지) 반영.
- 예상 blocker 후보(레드팀이 팔 자리): ①XGB lookback 버퍼가 계약을 안 깨고 서버측 상태로 성립하는지·초기(8행 미만) 요약의 skew ②arm-2 잔차가 실제로 network+직렬화로 좁혀지는지 ③관측성 게이트가 예측 로직 불변을 grep으로 증명 가능한지 ④통제 arm 잔차의 state 기여를 메모리 분해가 실제로 분리하는지 ⑤순차 실행이 하드웨어 상태(캐시·thermal) 차이를 남기는지 ⑥`.ubj` best_iteration 복구·주입 구현(핸드오프)이 챔피언 절단을 실제로 재현하는지.
