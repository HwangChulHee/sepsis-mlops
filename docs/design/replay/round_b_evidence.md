...
- **번들**: `deploy/artifacts/gru_vitals → gru_vitals@v0-base` (실재 alias, export/train 불필요).
  - `meta.json`: `featureset=vitals`, `input_dim=9`, `tau=0.5732471412047744`, `hp.hidden=128`,
    `run_id=8de08edb612749b2a45d3a029fac3123` (진짜 학습 모델 — reports/h2_results.md GRU/vitals 셀렉트).
...
PYTHONPATH=src uv run python scripts/replay_patient.py \
  ...
  | tee reports/replay/replay_p000018_septic.txt
...
- **일화(anecdote)지 성능 지표 아님.** 환자 3명 = 사례일 뿐. AUROC/utility/민감도-특이도 **주장 금지**.
  지표는 reports/h2_results.md(GRU/vitals util 0.4087 셀렉트)와 H3 cross-site 평가의 몫.
...
- `reports/replay/replay_p000018_septic.txt` — septic p 시퀀스(raw 증거, 134줄).
- `reports/replay/replay_p000001_control.txt` — 위양성 control(54줄).
- `reports/replay/replay_p000002_control.txt` — 저위험 control(23줄).
- 이 문서(`design/replay/round_b_evidence.md`).
