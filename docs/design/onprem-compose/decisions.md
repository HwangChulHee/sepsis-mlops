...
> **상태**: 설계부 v2 — 예열 라운드(R0) blocker 4건 반영, 정식 review-loop 대기.
> **개정 이력**
...
- **결정**: serving(읽기)·console-api(쓰기)가 공유하는 번들 저장소를 **호스트 bind mount** `./deploy/artifacts` → 컨테이너 `/app/deploy/artifacts`로 둔다(named volume 아님). 최초 번들은 **`h4s_export_bundle` 1회 수동 실행**으로 seed한다(Compose `up` 전 선작업). K8s의 공유 PVC(RWO)를 bind mount로 번역한 것.
- **근거 + 출처등급**:
...
- **미결/옵션**: bind mount 권한 — serving은 K8s에서 비-root(uid 10001)였음. Compose 보안 이식 수준(결정 6)에 따라 호스트 폴더 소유권·`user:` 지정 여부가 갈림. export가 만든 파일 소유권과 컨테이너 실행 uid 정합은 핸드오프에서 확정 [검증 필요: WSL2 bind mount uid 매핑].
- **검토 요청 항목**: 상대 심링크가 bind mount 경계를 넘어 컨테이너 안에서 실제로 해석되는지(WSL2에서 실측 필요), seed 안 된 상태로 `up` 했을 때 serving이 graceful 실패하는지 크래시루프인지.
...
- **미결/옵션**: "최소"의 정확한 경계(비-root만? cap drop 포함?)는 핸드오프. 하드닝 백로그 항목 목록화 필요.
...
- **결정**: `depends_on` + healthcheck로 순서 보장 — (1) serving이 healthy(부팅 캘리브레이션 완료) 후, (2) console-api(lifespan에서 alias↔감사 화해), (3) front nginx는 web+api 뜬 뒤. serving healthcheck는 `/health`, 부팅 예산은 K8s startupProbe(~300s)에 대응하는 `start_period`로.
- **근거 + 출처등급**:
...
- **★seed 미완 시 실패 비대칭 (B-R0-3)**: seed는 결정 2의 수동 선작업이나, 누락 시 두 서비스 거동이 갈린다 — **console-api는 graceful**(빈 alias·빈 audit이면 `_reconcile_or_seed`가 어느 분기도 안 타고 no-op 리턴) [확인됨: `service.py:135-148` — `alias_target=None & last=None`이면 무동작]. **그러나 serving은 graceful 아님** — 빈 alias 해석/번들 로드에서 예외 → `/health` 무한 실패 → **크래시루프**(원인 불명확) [검증 필요: `app.py` `_load_all`의 실제 예외 메시지·경로]. 따라서 serving에 **명시적 seed precondition 체크**(부팅 초입에 alias 존재 확인 → 없으면 "seed 필요: `uv run python -m scripts.h4.h4s_export_bundle`" 명확 에러로 즉시 종료)를 두어, 크래시루프 대신 **읽을 수 있는 실패**로 만든다 [우리 결정]. (체크 위치·메시지 구현은 핸드오프.)
- **미결/옵션**: `restart` 정책(`unless-stopped`) + healthcheck 실패 시 자동 재시작 여부 — Compose는 healthcheck unhealthy로 자동 재시작 안 함(autoheal 별도) [확인됨: Compose 동작]. POC는 restart만, hang 감지는 한계 명시. serving precondition 실패는 `restart: unless-stopped`와 결합 시 재시작 반복될 수 있으니, precondition 실패는 **비재시작 종료 코드**로 구분할지 핸드오프에서 결정.
- **검토 요청 항목**: serving `start_period`가 최악 부팅(캘리브레이션 스로틀)을 커버하는지, console-api가 serving 없이도 부팅되는지(승인 시에만 serving 호출하므로 부팅 의존은 느슨해야), seed precondition 체크가 크래시루프를 명확 실패로 바꾸는지.
...
- **★console↔serving featureset 불일치 (B-R0-4)**: console-api는 부팅 시 `CONSOLE_FEATURESETS` **전체를 순회**하며 화해/seed·`list_versions`를 노출한다 [확인됨: `service.py:159` `for fs in CONSOLE_FEATURESETS`]. 이 목록에 `vitals_labs`가 포함돼 있는데 serving은 `vitals`만 띄우면, **콘솔 UI에 active version 없는 featureset이 노출**되어 운영자를 혼란시킨다(승인 대상은 있는데 서빙 안 되는 유령 featureset). 따라서 POC는 **`CONSOLE_FEATURESETS`도 `vitals` 단일로 맞춘다**(serving과 정합). 두 featureset을 다 노출하려면 serving도 둘 띄워야 하며 그건 결정 8 범위 외 [우리 결정]. (`CONSOLE_FEATURESETS` 주입 방식·기본값은 핸드오프에서 확인 — env인지 상수인지.)
...
- **결정**: serving 컨테이너의 CPU·메모리를 **Compose 최상위 키(`cpus:`, `mem_limit:`)로** 건다. **`deploy.resources.limits`는 쓰지 않는다** — 그것은 Swarm 전용이라 `docker compose up`(비-Swarm)에서 **조용히 무시**된다. 값은 K8s 매니페스트와 정렬:
...
- **근거 + 출처등급**:
  - **`deploy.resources`는 `docker compose up`에서 무시됨** = Compose의 고전 함정 [확인됨: Compose 스펙 — `deploy` 키는 Swarm 오케스트레이터용, 비-Swarm 실행 시 리소스 제한 미적용]. K8s `resources.limits`를 순진하게 `deploy.resources`로 번역하면 컨테이너가 **호스트 전체 코어를 봄**.
...
- **고려한 대안**: (i) `deploy.resources` 사용 — Swarm 아니면 무시(기각, blocker 원인). (ii) 제한 아예 없음 — 호스트 전체 코어 노출로 thrash + reload OOM 방치, 부하 숫자 오염(기각). (iii) `docker compose` 대신 Swarm 모드 — 오케스트레이터 도입은 "K8s 안 쓰기로 한" 스코프와 어긋나고 과함(기각).
...
> BLAS 스레드 캡(`OMP_NUM_THREADS` 등)은 CPU 할당과 정렬 — 작은 추론 배열에 스레드 과투입 경합 방지 + 부팅 캘리브레이션 스로틀 방지 [확인됨: `deployment.yaml` env 주석]. Compose에서도 유지하되 **CPU 제한은 반드시 최상위 `cpus:`로**(결정 9 — `deploy.resources`는 무시됨).
...
- **best_iter/수치 재현** = xgboost 3.3.0(uv.lock) 전제 — 이미지 버전 고정 유지 [확인됨: `deploy/Dockerfile` torch 버전 고정 주석].
...
3. **기동 순서**가 seed 미완/캘리브레이션 지연 등 최악 케이스에서 크래시루프 대신 graceful 실패하는가? → **R0 부분 해소**: console-api graceful, **serving은 precondition 체크 신설로 대응**(결정 7, B-R0-3). 실제 예외 메시지 [검증 필요].
