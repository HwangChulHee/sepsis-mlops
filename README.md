...
운영 **H4(서빙·드리프트·재학습)** ✅ — **MLOps 루프 폐쇄** · **운영 콘솔**(champion-challenger
거버넌스, K8s E2E 실측) ✅ · **리플레이어**(실데이터 궤적 재생 → 실모델 관측) ✅. 설계 전문은
[`docs/design/`](docs/design/README.md), 서빙·확장 인프라 결정은 [`docs/adr/`](docs/adr/serving-scaling.md).
...
문서는 [`docs/design/`](docs/design/README.md), 측정 결과는 [`docs/reports/`](docs/reports/).
...
## 운영 콘솔 — champion-challenger 거버넌스

H4 재학습이 새 버전을 만들면 남는 질문은 모델이 아니라 **거버넌스**다: *누가* 승인하고,
*무엇으로* 판정하고, *어떻게* 안전 교체하며, *무엇이* 기록에 남는가. 운영 콘솔은 H4의
human-in-the-loop 게이트를 **하나의 화면**으로 구현한다 — 버전 리스트 → 클릭 → 게이트 수치 +
승인/롤백 + 감사. 한 사이클(설계 → 백엔드 TDD → 프론트 → 관측 → K8s 실측 E2E)을 닫았다.
설계 전문은 [`docs/design/console/decisions.md`](docs/design/console/decisions.md).

| 레이어 | 구현 | 핵심 |
|---|---|---|
| **감사** | SQLAlchemy append-only ORM | `before_flush` + `do_orm_execute` **이중 훅**으로 UPDATE/DELETE 차단(bulk 우회까지). 전용 칼럼(ts·event_type·gate_passed) + `gate_snapshot` JSON 박제 |
| **API** | `/console` 5개 | 읽기 3(`versions`·`versions/{v}`·`audit`) + 쓰기 2(`approve`·`rollback`). deploy/validate/bundle을 **재노출만** 하는 얇은 어댑터(ValueError→422·PermissionError→403) |
| **검증 게이트** | `validate.py` | ①B-holdout 성능 ②A-val 무회귀(=catastrophic forgetting, eps 0.02) ③**`cross_site_claim=false`**(정직성 플래그) |
| **권위** | FS alias = champion | DB 아님 — 부팅 시 alias↔감사 **reconcile**, 안전 플로우 = swap → audit |
| **프론트** | React+Vite+TS, 의존성 0 | 결정3 위계(게이트 **수치 크게** / 판정 작게), 버킷색(champion·challenger·archived) |
| **관측** | Grafana G3·G4 | 서빙 SLO(지연 p95/p99·처리량·알람률) + 헬스(up/down·생존) |
| **배포** | K8s | console-api · console-web · Ingress(동일 출처 `/`→web·`/console`→api, CORS 제거) · 공유 PVC |

**★ 실측 E2E (minikube):** 단위 테스트가 절대 못 잡는 **다중 파드 전파 사슬**을 합성 번들
(구조 검증 전용, **성능 주장 0**)로 실증 — 콘솔에서 `synthC` 승인 → 공유 PVC alias 교체 →
serve `/admin/reload` → **`/health.run_id`가 synthA→synthC로 실제 변경**(propagation confirmed).
부수 수확: serve Docker 이미지의 잠복 버그(pandas·scipy 누락 → 컨테이너 import 실패)가
**E2E로만 드러남**(단위는 로컬 pandas로 GREEN이라 못 잡음).
...
녹화된 환자 `.psv`의 시간 행을 **재생 버튼 누르듯** 서빙 `/predict`에 시간 간격대로 흘려보내는
장치. GRU가 causal·m2m이라 서버가 환자별 hidden state를 이어받으며 위험도 `p`가 시간 따라
오르내린다. 설계: [`docs/design/replay/`](docs/design/replay/).

- **엔진**: 무상태(환자 분리는 서버 hidden state + `patient_id` 플러밍). 시간 압축 `--speed`
...
실제로 어떻게 행동하는지**(텍스처)가 드러난다. 진짜 모델(`gru_vitals`, τ=0.5732)로
in-distribution A 환자 3명 관측:

| 환자 | 라벨 | 행 | onset_t | p 범위 | alarm | 관측 |
|---|---|---|---|---|---|---|
| **p000018** | septic | 134 | 125 | 0.46–0.95 | 121/134 | **t=3부터** 상시 고위험 — 라벨 onset(t=125) 한참 전 |
| **p000001** | non-septic | 54 | — | 0.30–0.82 | 14/54 | 후반 0.82까지 상승 → **지속 위양성** |
| **p000002** | non-septic | 23 | — | 0.20–0.46 | 0/23 | 저공 횡보 — 깨끗한 음성 |

**핵심 발견:** septic 환자조차 "조기 경보"가 아니라 **"상시 경보"**다(134시간 중 121시간 알람,
입원 3시간째부터). control p000001은 onset 없이 14번 울렸다. 즉 τ=0.5732에서 이 모델은
**과경보 성향** — 그런데 이게 바로 배포된 패혈증 모델을 무너뜨린 실패 모드(Epic Sepsis Model의
**알람 피로**)다. 리서치 Q5의 추상을 리플레이어가 **실데이터 위 실물 증거**로 박았다.
증거 전문: [`docs/design/replay/round_b_evidence.md`](docs/design/replay/round_b_evidence.md).

> **정직한 한계:** 환자 3명 = **일화이지 성능 지표 아님**. AUROC/utility 주장은 H2·H3의 몫
> (이건 *질적 텍스처* 관측). in-distribution(A) 관측, `cross_site_claim=false` 불변.
> τ는 번들 동결값 — 운영 중 라이브 재선택은 누수.
...
범례는 [`docs/research/`](docs/research/README.md) 참고.

| | 질문 | 한 줄 결론 | 문서 |
|---|---|---|---|
| **Q1** | 어떤 모델? | baseline=**XGBoost/LightGBM**, 메인(운영)=**GRU**, 욕심=Transformer. 진짜 난관은 모델이 아니라 **병원 간 일반화**. | [01_models](docs/research/01_models.md) |
| **Q2** | 뭘 먹이고 빈칸은? | 활력징후+나이·성별 기본, 피검사는 핵심만. 결측은 **모델별 분기**(XGBoost는 NaN 그대로, GRU는 forward-fill). **0-fill 금지.** 결측 마스크는 누수 위험으로 **기본 OFF·옵트인**. | [02_features_missing](docs/research/02_features_missing.md) |
| **Q3** | 성공을 뭘로 재나? | 정확도 버림(불균형), AUROC 보조, **PR-AUC 1차**, **utility score 정식**(1=완벽·0=무행동·음수=해로움), **cross-site(A→B)**. | [03_evaluation](docs/research/03_evaluation.md) |
| **Q4** | 누수·일반화 함정? | 컨닝 길 4개 — ①시간단서 ②**치료행동 누수** ③병원 일반화 ④환자분할. ①④는 처리, **②③은 인지+운영으로 관리**. | [04_leakage_generalization](docs/research/04_leakage_generalization.md) |
| **Q5** | 어떻게 굴리나? | 성패는 모델이 아니라 **운영**이 가른다(Epic 실패 vs TREWS 사망률 18%↓). 실시간 서빙·알림률·**입력 드리프트 감시**·재학습(피드백 루프 인지)·배포 전 cross-site. | [05_serving](docs/research/05_serving.md) |
...
작성 내용은 **[docs/reports/eda_findings.md](docs/reports/eda_findings.md)**(EDA)와
**[docs/reports/smoke_findings.md](docs/reports/smoke_findings.md)**(스모크) 참고. EDA 핵심 수치:
