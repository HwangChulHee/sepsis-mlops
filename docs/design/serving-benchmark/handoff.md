...
GRU 서빙과 **같은 `/predict` 계약**을 따르는 **XGBoost 최소 서빙 앱**을 세운다. 핵심 난점 셋 — (1) XGB는 stateless가 아니라 **환자별 raw 이력 버퍼**를 서버측에서 유지해 시간 요약을 재구성해야 하고, (2) 챔피언과 **동일한 출력**을 내도록 재현해야 하며, (3) latency 지표가 GRU와 **같은 범위를 재도록** 대칭을 맞춰야 한다(경계 세부는 §B5). 이 1차 핸드오프는 **XGB 서빙만** 세운다(벤치 측정·비교는 2차). (구현 개념 — 버퍼 길이·요약 통계·트리 절단 반복수 등은 §B에만 두고, §A는 관측 상수/행동으로만 검증한다.)
...
XGB 서빙은 **환자별로 최근 raw 행들을 기억**한다. 매 `/predict`가 오면 그 환자의 과거 행 + 이번 행을 **함께 써서(시간 이력 반영)** 추론한다 — 같은 한 행이라도 앞선 이력에 따라 다른 `p`가 나온다.
...
- **A2-c (이력이 짧을 때 = 학습과 동일한 처리)**: 첫 몇 요청(이력이 아직 짧을 때)에도 500 없이 유효한 `p`를 반환한다. 부족분을 **0으로 채우지 않는다**. 이 "0-fill 아님"은 별도 화이트박스 검사 없이 **A3-a 골든 재현으로 관측 교차검증된다**: 골든의 앞쪽 요청들은 이력이 짧은 상태에서 나온 값이고, 골든 기대값은 부족분을 학습과 동일한 결측 처리로 산출했다. 서버가 부족분을 0으로 채우면 마지막 `p`가 골든과 불일치 → A3-a가 FAIL로 잡는다.
...
- **A3-a (골든 재현)**: 위 5행을 순서대로 보낸 뒤 **5번째(마지막) 응답**의 `p` = **`0.70981 ± 1e-4`**, `alarm` = **true**(동결 `tau` 기준). 이 값은 동결 챔피언의 출력이다.
  - **A3-a-mid (궤적 고정 — 거짓 통과 방지)**: 같은 `S_vitals`를 순서대로 보낼 때 **3번째 응답**의 `p` = **`0.24634 ± 1e-4`**, `alarm` = **false**. 마지막 값 하나만 대조하면 절단 외 다른 구현 오류가 우연히 `0.70981`에 착지해 거짓 통과할 수 있으므로, 궤적 중간 지점을 함께 고정한다. (동결 아티팩트 산출 관측 상수 — `S_vitals` 앞 3행 이력 후 절단 예측 `0.2463367…`, `.venv` xgboost 3.3.0 실측 [확인됨].)
...
- **best_iter**: `.ubj`에 임베드됨. `xgb.Booster().load_model(<.ubj>)` 후 `b.best_iteration` → **105**(vitals) / **149**(vitals_labs)로 실측 확인. 실행은 **실효 핀 = `uv.lock`(xgboost==3.3.0) 락 환경**(`uv run`)에서 — `requirements*.txt`는 부재, `pyproject.toml:19`는 `>=3.3.0` 부유 하한이라 버전 고정은 `uv.lock`뿐(결정론 경계는 §B3.1). run 스토어 조회 불필요(이 저장소 mlruns는 artifacts-only pruned, `grep best_iter mlruns/`=0).
...
§A3의 골든 상수는 **동결 `.ubj` + `lookback_summary` + best_iter 절단**으로 산출·**독립 재현으로 확증됨** [확인됨]. 최초 산출은 reviser가 `.venv/bin/python`(xgboost 3.3.0)으로 실측했고, 이후 **지휘자가 동일 락 환경(`.venv`, xgboost 3.3.0)에서 저장소 함수 그대로 5개 상수를 전부 재현·일치 확인**했다 — vitals `0.7098076`/`0.6923320`/`0.6486735`, vitals_labs `0.8335611`/`0.8228457` [확인됨]. 즉 골든은 단일 실측이 아니라 **두 실행자의 독립 재현으로 확증**된 상수다. (spec-writer가 상수를 하드코딩하기 전 아래 락 환경에서 1회 재확인하는 것은 여전히 권장 — 재현 절차가 문서화돼 있어 부담 없음.) 재현 경로:
...
- A3-a-mid용: `S_vitals` **앞 3행**을 `(3,9)` 버퍼로 요약(`lookback_summary`)→마지막 행 절단 예측 → `p=0.2463367…`(반올림 `0.24634`), `tau` 미만이라 `alarm=false`.
- A2-a용: `S_vitals` **마지막 행 X 단독**을 1행 버퍼로 요약→절단 예측 → `p=0.6486734…`(반올림 `0.64867`).
...
- **실효 핀·실행 환경**: 이 저장소에 `requirements*.txt`는 **없다**. `pyproject.toml:19`는 `xgboost>=3.3.0`(부유 하한)이라 그 자체로는 버전을 못 고정한다. **유일한 실효 핀은 `uv.lock`(resolved `xgboost==3.3.0`)이며, 골든 재현·RED 테스트는 반드시 `uv run`(= `uv.lock` 락 환경)에서 실행한다** [확인됨: `uv.lock:3405-3406` version 3.3.0]. (원하면 `pyproject.toml:19`를 `xgboost==3.3.0`으로 조여 pip 경로 이탈까지 막는 옵션도 있으나, 락 환경 실행이 1차 방어선.)
- **결정론 (양방향 경계)**:
  - *상한(감별 안전)* — eps `1e-4`는 절단↔비절단 간 최소 gap `~0.0107`(vitals_labs)보다 두 자릿수 작아, "절단이 걸렸는지"를 안전하게 감별한다.
  - *하한(재현 안전)* — eps `1e-4`가 부동소수 지터보다 큰지는 **`uv.lock` 락 환경(xgboost 3.3.0, 동일 플랫폼/BLAS/패치버전) 안에서만 보장**한다. 트리 부스터 추론은 결정론적이라 같은 락 환경에선 비트 단위 재현되지만(지휘자·reviser 2회 독립 재현으로 확인), **락 환경 밖(다른 xgboost 패치버전·다른 BLAS·다른 플랫폼)에선 지터가 eps를 넘을 수 있으므로 보장하지 않는다.**
  - **경계**: 따라서 §A3의 골든 상수(RED)는 **`uv.lock` 락 환경 전용**이다. xgboost 버전이 오르거나 락이 갱신되면 **골든을 재산출해 다시 박아야 한다**(위 재현 경로로 5개 상수 재확정 → §A3·§A2-a 갱신).
